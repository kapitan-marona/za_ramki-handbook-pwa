...
shutil.copy2(backend, backend.with_suffix(".js.bak_micro4"))
shutil.copy2(api, api.with_suffix(".js.bak_micro4"))
...
new_api = '''    return await ZRBackend.taskActivity.listByTask(taskId);
'''

if old_backend not in backend_text:
    raise SystemExit("Backend allowlist anchor not found")

if old_api not in api_text:
    raise SystemExit("Planner API fetchTaskActivity anchor not found")
