...
Views.Measurements = (() => {
  ...
  async function open(){
    ...
    viewer.innerHTML = `
      <style>
        .m-actions{display:flex;flex-wrap:wrap;gap:10px;margin:14px 0 18px}
        .m-actions .btn{display:inline-flex;align-items:center;gap:8px}
        .m-note{border:1px solid var(--border);background:rgba(255,255,255,.03);padding:12px 14px;border-radius:14px}
        .m-note b{color:var(--text)}
        .m-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;margin:10px 0 16px}
        .m-grid img{width:100%;height:auto;border-radius:14px;border:1px solid var(--border);background:rgba(255,255,255,.02)}
        @media (max-width: 900px){ .m-grid{grid-template-columns:1fr} }
        .m-step h2{margin-top:18px}
        .m-sub{color:var(--muted);margin-top:6px}
        .m-small{color:var(--muted);font-size:.95rem}
      </style>
