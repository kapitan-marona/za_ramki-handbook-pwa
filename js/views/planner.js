...
Views.Planner = (() => {
  ...
  async function show(){
    ...
    const renderTasksList = (tasks) => {
      ...
      const selected = parseSelectedId();
    ...
    const renderViewer = (tasks) => {
      const selectedId = parseSelectedId();
      if(!selectedId){
        if(!tasks || tasks.length === 0) renderEmptyViewer();
    ...
    async function fetchTasksForTab(tab){
      ...
      // UI-side publish rule for staff (для предсказуемости)
      if(role !== "admin"){
        // показываем start_date null ИЛИ start_date <= today
        // supabase-js не умеет OR красиво без rpc — делаем после fetch
      }
    ...
    // --- render flow ---
    renderHeader(0);
    const tasks = await fetchTasksForTab(state.tab);
